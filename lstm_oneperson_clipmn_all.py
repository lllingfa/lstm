import sys
from getdata_oneperson_clipmnr_tree_all import getdata
...
model.load_weights('model/weights-oneperson-clip-20-lstmtree-mnr-all-2017-04-11-14:43:31.hdf5')
for layer in model.layers:
    layer.trainable = True
...
checkpointer = ModelCheckpoint(filepath='model/weights-oneperson-clip-20-lstmtree-mnr-all-'+str(times)+'.hdf5', verbose=1, save_best_only=True)
#write to log
#csv_logger = CSVLogger('/model_log/training.log')
model.fit(X_train, y_train,nb_epoch = 40,batch_size=64,verbose=1,callbacks=[checkpointer],validation_split=0.05)
